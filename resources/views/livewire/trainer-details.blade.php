            ...
            @if($trainer)
                <div class="bg-white rounded-xl shadow-xl overflow-hidden">
                                    ...
                                    @if(isset($trainer->email) && $trainer->email)
                                        ...
                                        <ul class="space-y-2">
                                            <li class="flex items-center text-gray-700">
                                                ...
                                                <a href="mailto:{{ $trainer->email }}" class="text-blue-600 hover:text-blue-800 hover:underline transition-colors">
                                                    {{ $trainer->email }}
                                                </a>
                                            </li>
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0">
                        <a href="{{ route('trainers.list') }}" wire:navigate class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                        ...
                        @if(Auth::check())
                            @if(Auth::user()->isTrainer() && Auth::user()->id != $trainer->id)
                                <a href="{{ route('reservation.create.trainer', ['trainerId' => $trainer->id]) }}" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                    </svg>
                                    {{ __('trainers.make_reservation') }}
                                </a>
                            @elseif(Auth::user()->isTrainer() && Auth::user()->id == $trainer->id)
                                <span class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-gray-400 text-white rounded-md cursor-not-allowed">
                                    ...
                                    {{ __('trainers.cannot_book_yourself') }}
                                </span>
                            @elseif(Auth::user()->isUser())
                                <a href="{{ route('reservation.create', ['trainerId' => $trainer->id]) }}" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
                                </svg>
                                {{ __('trainers.login_to_reserve') }}
                            </a>
                        @endif
                    </div>
                </div>
