            ...
            <div class="flex space-x-4 items-center">
              @auth
                ...
                <div x-data="{ dropdownOpen: false }" class="relative z-20">
                  <button @click="dropdownOpen = !dropdownOpen" class="text-gray-300 px-3 py-2 rounded-md hover:bg-gray-700">
                    {{ Auth::user()->name }}
    ...
    <footer class="bg-gray-800 text-center py-6 text-gray-600">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div x-data="{ showModal: false, message: '' }"
          ...
          <!-- Subscription Form -->
          <form action="{{ route('subscribe.store') }}" method="POST" class="flex justify-center mt-4 z-50 relative">
            @csrf
            ...
            <button type="submit" class="px-4 py-2 bg-orange-500 text-white rounded-r-full hover:bg-orange-600 transform transition duration-300 hover:scale-105">
              Subscribe
            </button>
          </form>
